...
4. ■SCORESの5項目（構図・構成、光・色彩、ストーリー、技術・露出、独自・世界観）は写真ごとに個別に分析し、1〜5の数値（および対応する★記号）を独自に算出して必ず全5項目出力してください。
...
(※SCORES出力例: ・構図・構成  : ★★★☆☆ (3/5) のように必ず5項目すべて★記号と(数値/5)形式で出力すること)
...
"""
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # 検知強化: 拒否応答・TITLE欠如・SCORES項目（・）欠如の場合は自動リトライ
            if "申し訳ありません" in content or "■TITLE:" not in content or "・" not in content:
                print(f"⚠️ 応答フォーマット不備を検知 (試行 {attempt}/{max_retries})。再試行します...")
