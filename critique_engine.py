...
def generate_critique(
    ...
    # 時間帯ファクトの表示補正（EXIFなしの場合の誤判定防止策）
    if time_zone_fact in ["不明", "なし", ""]:
        time_zone_display = "不明 (※EXIF情報なし。安易に『夕日』『夕暮れ』『夕焼け』と決めつけず、光の質感・コントラストを中心に描写すること)"
    else:
...
2. 時間帯ファクト（{time_zone_display}）を尊重してください。時間帯が『不明』の場合は、安易に『夕日』『夕焼け』『夕暮れ』と決めつけることは一切厳禁です。朝の光や印象的な光線美として描写するか、時間帯を断定しない美しい表現を徹底してください。
...
"""
    ...
    # =========================================================
    # Phase 2: 本文【1】〜【7】生成
    # (ハッシュタグ指示 #カメラ_{camera_tag} #レンズ_{lens_tag} を完全保持)
    # =========================================================
...
3. 【光と陰影の整合】: 時間帯ファクト（{time_zone_display}）を遵守してください。時間帯が『不明』の場合は安易に『夕日』『夕焼け』等と断定・決めつけず、画面に現れている光の質やコントラスト、雰囲気を客観的に描写してください。
4. 【具体的なアクション指導】: アドバイスでは具体的な動作や数値で示してください。
5. 【確定評価の維持】: 提示された事前確定の■SCORESの内容と整合性を保ちながら【1】〜【7】の文章を記述してください。
6. 【自動タグの厳格付与】: 【7】の先頭には必ず `#カメラ_{camera_tag} #レンズ_{lens_tag}` をそのまま出力し、続けて写真の被写体・光・質感に応じたハッシュタグを8〜12個出力してください。
