...
6. 【動的ハッシュタグ生成】: ## 【7. 自動タグ】 では固定の型をコピペせず、写真に写っている被写体、場所、季節、光・色彩の印象、空気感、ストーリー、テーマ等を反映したハッシュタグ（例: #航空機 #雪 #冬 #空港 #旅 など）を8〜12個程度、提示された写真に合わせて独自に生成してください。
...
## 【7. 自動タグ】
(写真に写っている被写体、場所、季節、空気感、ストーリー・テーマ等に応じたハッシュタグを8〜12個生成してください)
"""
    else:
        # 簡易版用軽量プロンプト (メタデータ条件考慮 + 高速カード生成要素のみ出力)
        prompt = f"""あなたは写真表現と撮影技術を深く探求するプロの写真評論家・フォトブック編集者です。
与えられた写真および以下のメタデータを観察し、カード画像生成に必要な4項目（TITLE, SUMMARY, SCORES, CRITIQUE_SUMMARY）のみを即座に作成してください。
...
【撮影意図・見出し】
- 見出し: {content_headline}
- 撮影意図・コメント: {user_intent}
...
・独自・世界観: [写真に応じた★評価] ([1〜5の数値]/5)
(※SCORES出力例: ・構図・構成  : ★★★☆☆ (3/5) のように必ず★記号5文字と(数値/5)形式で出力すること)
■CRITIQUE_SUMMARY: 否定的なコメントや数値を使えず、「本人が意識していないかもしれないが非常に効果的なポイント」や「良い点」を主体に、読者の好奇心を煽る文章を70〜80文字程度で記述してください。
"""
