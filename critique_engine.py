...
def generate_critique(
    ...
    # (時間帯単語の直接使用を禁止し、光と構図の美しさにフォーカスさせる)
    # =========================================================
...
"""
    ...
    # =========================================================
    # Phase 2: 本文【1】〜【7】生成
    # (時間帯情報は背景知識として与えつつ、単語の多用を抑え光の質を描写させる)
    # =========================================================
...
3. 【光と陰影の具象的描写】: 撮影時間帯ファクト（{time_zone_fact}）を前提知識として考慮しつつも、『朝日』『夕日』『夕焼け』『夕暮れ』『夕映え』『夕景』『夜景』『黄昏』などの単語を安易に連呼することは避けてください。単語のラベル貼りではなく、「光の差し込む角度」「明暗のコントラスト」「グラデーションの推移」「シャドウの深度」など具体的な光と色彩の表現として文章を構築してください。
4. 【具体的なアクション指導】: アドバイスでは具体的な動作や数値で示してください。
5. 【確定評価の維持】: 提示された事前確定の■SCORESの内容と整合性を保ちながら【1】〜【7】の文章を記述してください。
6. 【自動タグの厳格付与】: 【7】の先頭には必ず `#カメラ_{camera_tag} #レンズ_{lens_tag}` をそのまま出力し、続けて写真の被写体・光・質感に応じたハッシュタグを8〜12個出力してください（※安易な時間帯ラベルタグの乱用は控えること）。
