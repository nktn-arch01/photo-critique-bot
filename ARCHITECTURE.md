...
- **Guided の方位受理**: プロンプト禁止だけではカード（SUMMARY）や【1】〜【4】に「夕暮れ」が残る。アプリが東／西（一日の前半／後半）を確定し、`check_output_east_west_reversal` を Phase1 / Phase2 の受理条件にする。既存 `max_retries` で観察ごと差し戻す。文面の差し戻しに通信用の待ちは掛けない。朝夕語の単語置換はしない。失敗は `CritiqueContractError`（通信障害ではない）。LINE / Desktop の生成経路は変えない。
