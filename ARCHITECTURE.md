...
- `light_context.py`: 地域・タイムゾーン・時間帯を禁止語なしの光の手掛かりへ変換する共通層（Guided プロンプトと LINE／Desktop Phase 2 が共用）。
...
- **Guided の `time_band`**: UI 表示用の不定時法ラベル（夜明け／夕暮れ／夜 等）。プロンプトへは生ラベルを入れず、`light_context.py` で禁止語なしの「光の手掛かり」へ変換する。地域・タイムゾーンも同じブロックで渡す。
- **Phase 1（カード用要素）**: LINE／Desktop は日時・時計帯を与えない。Guided は撮影日時を与えず、地域・タイムゾーン・光の手掛かりのみ（画面の光が手掛かりと食い違うときは画面優先）。時間帯単語の使用は【一切厳禁】。暗い画面でも「夜」「夕」と推測して書かせない。
- **Phase 2（長文本文）**: 撮影日時と光の手掛かり（Guided は地域・タイムゾーンも）を提示するが、画面の印象でヒントを上書きするラベル貼りを禁じ、「光の照射角度」「明暗のコントラスト」「色彩グラデーション」「シャドウの深度」など具体性のある光の表現へ変換させること。
