...
- `guided_web/`: **【P2-2 Guided Web】** ローカル FastAPI＋3画面（選ぶ／読む／振り返る）。日常起動は `LuminaNotesGuided.app`（ターミナルなし。画面は `.command` と同じ `python3` + `/usr/bin/open`。終了は画面の「終了」。画面が無ければサーバも止まる。シェル `.app` は Dock に出ない）。保険は `LuminaNotesGuided.command` / `scripts/run_guided_web.sh`。演算中だけ `caffeinate -i`。作業ルールは `AGENTS.md`。PR #21 の枝で開発中。
