...
- **Guided の方位受理**: プロンプト禁止だけではカード（SUMMARY）や【1】〜【4】に「夕暮れ」が残る。アプリが東／西（一日の前半／後半）を確定し、`check_output_east_west_reversal` を Phase1 / Phase2 の受理条件にする。既存 `max_retries` で差し戻し、それでも朝夕語が残れば方位を名乗らない『低い光』へ置き換える。LINE / Desktop の生成経路は変えない。
