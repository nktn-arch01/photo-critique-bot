...
- `guided_web/`: **【P2-2 Guided Web】** ローカル FastAPI＋3画面（選ぶ／読む／振り返る）。日常起動はアプリケーションフォルダの `LuminaNotesGuided.app`（`scripts/install_guided_app.sh`。ターミナルなし。終了は画面の「終了」。画面が無ければサーバも止まる。シェル `.app` は Dock に出ない）。保険はリポジトリ内の同名 `.app` / `LuminaNotesGuided.command` / `scripts/run_guided_web.sh`。演算中だけ `caffeinate -i`。作業ルールは `AGENTS.md`。PR #21 の枝で開発中。
...
- **「別のホストに乗り換える」は対症療法である。** 満たせない約束（シェル `.app` を Dock の本物アプリに見せる等）は最初の失敗で止め、仕様を正直にする。手順は `AGENTS.md` の Guided Mac 節。
...
- **Guided の光の手掛かり**: 選ぶ画面の `time_band`（不定時7段階＋夜）とは別に、講評へは太陽の方位・高度から作った `light_hint`（東の空／西の空、一日の前半／後半）を**覆せない事実**として渡す。朝日・夕日・夕暮れ等はヒントに含めない。画面を優先してよいのは光の強さ・色・影の形だけ。方位は見た目のオレンジで読み替えない。
- **Guided の方位受理**: プロンプト禁止だけではカード（SUMMARY）や【1】〜【4】に「夕暮れ」が残る。アプリが東／西（一日の前半／後半）を確定し、`check_output_east_west_reversal` を Phase1 / Phase2 の受理条件にする。既存 `max_retries` で観察ごと差し戻す。文面の差し戻しに通信用の待ちは掛けない。朝夕語の単語置換はしない。失敗は `CritiqueContractError`（通信障害ではない）。Phase1 の差し戻し文に【1】〜【7】や構図批評を混ぜない。撮影設定は Phase2 にだけ渡す。LINE / Desktop の生成経路は変えない。
- **Phase 1（カード用要素）**: 時間帯の先入観を防ぐため日時・時計帯をプロンプトに与えず、時間帯単語の使用を【一切厳禁】とする。暗い画面でも「夜」「夕」と推測して書かせない。（Guided は例外として `light_hint` の方位のみ渡す。）
