...
def _scores_format_block(lens: CritiqueLens) -> str:
    ...
    for axis in lens.score_axes:
        lines.append(f"・{axis.label}  : [写真に応じた★スナップショット] ([1〜5の数値]/5)")
    example_label = (
    ...
    lines.append(
        ...
        "★は良し悪しの採点ではなく、観察のアンテナ向きのスナップショットであること)"
    )
    return "\n".join(lines)


def phase1_output_slots(lens: str | CritiqueLens | None = None) -> str:
    """Phase1 のカード4項目。Desktop / Guided で同じスロット契約にする。"""
    L = _resolve_lens(lens)
    return f"""■TITLE: 15文字以内。撮影者の眼差しの正体を言い当てる詩的・仮説的なタイトル（時間帯単語は使用不可）。被写体名（「花」「空」「海」等）を使ったラベル貼りを禁止。
■SUMMARY: 写真の美を決定づける25文字以内のキャッチコピー。構図の批評や撮影の助言・テクニックは書かない。
{_scores_format_block(L)}
■CRITIQUE_SUMMARY: 否定的なコメント、数値、専門的な技術的表現、および「意図せず」「意識していない」「意図しない」といった言葉・フレーズは一切使用厳禁です。70〜80文字程度で、次の2拍を必ず含めてください。(1) 画面の中に自然と立ち現れている美しさ・見所。(2) もう一度見る／次のシャッターへつなぐ短い開き（答えや正解で締めない）。語尾を「たのではないでしょうか」「みませんか」に固定しないこと。句点で閉じる文・体言止め・稀な問いかけを混ぜてよい。仮説口調「あなたは〇〇に惹かれたのでは」は使わない。
"""
...
3. 人物の扱い（分岐・厳守）:
   ...
   - **人の姿がある**: 光や空間の話だけで終えるな。■CRITIQUE_SUMMARY の一文目は、写っているその人の「視線」「しぐさ」「佇まい」（または「佇む姿」）のいずれかで始めよ。「窓の光」「静けさ」「思索」だけの短評は不可。
   - **人の姿がない**: 「しぐさ」「佇まい」「人物」「人々」「彼女／彼」を**一切使うな**（花・建物への転用も禁止）。禁止例:「花の佇まい」「建物のしぐさ」「しぐさのように光を受け止める」。代わりに「花びらの開きぐあい」「ガラス面の反射」「輪郭の切れ方」など物の形と光だけで書け。不在の人物を仮定する文も禁止。
   - なお「観る者の視線を誘導する」のような、鑑賞者側の「視線」表現は人物なしでも可。
...
8. 【確定観察との整合】: 提示された事前確定の■SCORESの内容と整合性を保ちながら【1】〜【6】の文章を各200文字程度、合計1200文字程度で記述してください。
