...
def parse_dop_file(dop_path: Path) -> dict:
    ...
    try:
        ...
        if rank_m:
            ...
            info["rating_str"] = "★" * r + "☆" * (5 - r) + f" ({r}/5)"

        preset_m = re.search(r'PresetName\s*=\s*"([^"]+)"', content)
        if preset_m:
            info["preset_name"] = preset_m.group(1)

        headline_m = re.search(r'Headline\s*=\s*"([^"]+)"', content)
        if headline_m:
            info["content_headline"] = headline_m.group(1)

        caption_m = re.search(r'Caption\s*=\s*\[\[(.*?)\]\]', content, re.DOTALL)
        if caption_m:
            info["user_intent"] = caption_m.group(1).strip()
        ...
        if supp_m:
            ...
            info["other_categories"] = ", ".join(cats)
...
def extract_exif_metadata(img_path: Path) -> dict:
    meta = {
        "camera_model": "不明",
        "lens_model": "不明",
        "f_number": "不明",
    ...
    try:
        with Image.open(img_path) as img:
            ...
            if "ExposureTime" in exif_data:
                meta["shutter_speed"] = f"1/{int(1/float(exif_data['ExposureTime']))}s"
            if "ISOSpeedRatings" in exif_data:
                meta["iso"] = f"ISO {exif_data['ISOSpeedRatings']}"
            if "FocalLength" in exif_data:
                meta["focal_length"] = f"{int(exif_data['FocalLength'])}mm"
            ...
            if dt_str:
                try:
                    ...
                    hour = dt.hour
                    if 4 <= hour < 7:
                        meta["time_zone_fact"] = "早朝・黎明（日の出前後）"
                    elif 7 <= hour < 11:
                        meta["time_zone_fact"] = "午前（順光・斜光）"
                    elif 11 <= hour < 14:
                        meta["time_zone_fact"] = "昼間（トップライト・高コントラスト）"
                    elif 14 <= hour < 17:
                        meta["time_zone_fact"] = "午後（斜光・長シャドウ）"
                    elif 17 <= hour < 19:
                        meta["time_zone_fact"] = "夕方・マジックアワー（日の入り前後）"
                    else:
                        meta["time_zone_fact"] = "夜間・暗所（人工光・長時間露光）"
                except Exception:
                    meta["date_time"] = str(dt_str)
...
class PhotoAICritiqueApp:
    ...
    def start_processing_thread(self):
        ...
        # 💡 修正ポイント: ドット/._ で始まる隠しファイルを完全除外してカウント
        image_files = [
            f for f in target_dir.iterdir() 
            if f.is_file() and not f.name.startswith(".") and f.suffix.lower() in [".jpg", ".jpeg"]
        ]
        if not image_files:
            messagebox.showwarning("警告", f"対象フォルダ内にJPG画像が見つかりません:\n{target_dir}")
            return
        ...
        try:
            ...
            for idx, img_path in enumerate(image_files, 1):
                ...
                try:
                    ...
                    exif_meta = extract_exif_metadata(img_path)
                    
                    dop_path = img_path.parent / f"{img_path.stem}.dop"
                    if not dop_path.exists():
                        dop_path = img_path.parent / f"{img_path.name}.dop"
                    ...
                    dop_status = f"あり [評価: {dop_info.get('rating_str', 'なし')}] [Preset: {dop_info.get('preset_name', '標準')}]" if dop_path.exists() else "なし"
                    metadata_block = f"""file_name: {file_name}
...
contentHeadline: {dop_info.get('content_headline', 'なし')}
user_intent: {dop_info.get('user_intent', 'なし')}
Category: {dop_info.get('category', 'なし')}
OtherCategories: {dop_info.get('other_categories', 'なし')}
Keywords: {dop_info.get('keywords', 'なし')}"""
