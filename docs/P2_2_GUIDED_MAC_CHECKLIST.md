...
- 数秒後、ブラウザが開く（開かない場合は下記ステップ3で手動 URL）  
- ターミナルは **閉じない**（閉じるとアプリも止まります）  
...
ブラウザに **Lumina Notes** と、画面上部に **選ぶ / 読む / 振り返る** の文字があること。

開かないときは、ターミナルに出ている URL を Safari または Chrome のアドレス欄に貼り付け:
...
3. 画面下に **JSON 形式のパラメータ**（`image_id`, `time_band` など）が表示される  

| # | 確認内容 | PASS の目安 |
|---|----------|-------------|
| 3a | 3画面のラベル | 選ぶ・読む・振り返る が見える |
| 3b | 写真選択 | 選んだあと JSON が表示される |
| 3c | レスポンシブ | ブラウザ幅を狭くしても崩れず1列で読める |
