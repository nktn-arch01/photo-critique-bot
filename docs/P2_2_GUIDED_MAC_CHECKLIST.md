...
12. **Noteに書き出す**（右寄せ）→ Mac のフォルダ選択ダイアログで保存先を指定  
13. 書き出し成功後 **選ぶ** に戻る。保存先に `photo.jpg` / `card.png` / `note.md` / `session.json` がある  
...
| 4d | 振り返る・カード | 一言・思い★・ファイル名がカードに入る |
| 4e | ★・書き出し | 書き出し時にフォルダ選択→4ファイル作成 |
