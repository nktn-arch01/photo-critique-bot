...
10. **フォルダを選ぶ** → Mac のフォルダ選択ダイアログで保存先を指定  
11. 星 **☆☆☆☆☆** → タップで **★** → **Noteに書き出す** が押せる  
12. 書き出し成功後 **選ぶ** に戻る。保存先フォルダに `photo.jpg` / `card.png` / `note.md` / `session.json` がある  

| # | 確認 | PASS の目安 |
|---|------|-------------|
| 4a | 講評の言葉 | TITLE 相当の見出しと文章が出る |
| 4b | もう一度 | 写真が残る |
| 4c | 振り返る・カード | カード画像が見える |
| 4d | フォルダ選択 | 保存先パスが表示される |
| 4e | ★・書き出し | 星選択後に書き出し成功、ファイル4点ができる |
