...
| 3f | 講評の途中で戻る | 「いま写真を読んでいます…」のあいだに「もう一度」→ 選ぶの写真を選ぶ／クリア／言葉にするが動く。固まったままにならない |
...
13. 書き出し成功後、画面下にトースト（「書き出しました」）が出て **選ぶ** に戻る。次回は **前回の保存先** が初期表示される  
...
| 4f | ★・書き出し | `{名前}_LN.png` / `{名前}_LN.md` の2点のみ。成功はトースト＋選ぶへ |
...
**見てほしいこと**: サーバが止まり、ブラウザは更新しても繋がらなくなる。一時フォルダ（`/tmp/lumina_guided` 相当）は終了時に掃除されます。
