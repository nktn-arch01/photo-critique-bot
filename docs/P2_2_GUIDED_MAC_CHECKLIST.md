...
## `.app` のやり直し確認（A1〜A5）— オーナー完了宣言（2026-08-30）

単独起動・終了化は **完了**。Dock / WebKit / applet の続きは新しい依頼が出るまでやらない。以前の「アプリの窓」実装は破棄済み。いまは **`.command` と同じ python3** でサーバを立て、画面はいつものブラウザです。ターミナルは出ません。
...
| # | 結果 |
|---|------|
| A1 | PASS（2026-08-30）選ぶがブラウザで開く |

### A2 — 画面の「終了」で止める

Dock に **Lumina Notes Guided** は出ません。これは不具合ではなく、いまの `.app` の仕様です（中身はシェルで、macOS が Dock に残す種類のプログラムではありません）。終了は画面で行います。

1. **選ぶ** がブラウザで開いている  
2. 画面右上の **終了** を押す  
3. 「終了しました。このタブを閉じてください。」と出る  
4. そのタブを閉じる  
5. 同じアドレスを更新すると、繋がらない  

前回の実験で Dock のアイコンが跳ね続けているときは、そのアイコンを右クリック → **強制終了** してから、もう一度 `LuminaNotesGuided.app` を開いてください。新しい起動では跳ねません。
...
| 4f | ★・書き出し | `{名前}_LN.png` / `{名前}_LN.md` の2点のみ。成功はトースト＋振り返るに留まる。**フォルダ選択の Cancel はオーナー PASS（2026-08-29）**（ダイアログを閉じても落ちない・エラーにしない） |
| 4g | 空状態 | 写真の前に「読む」「振り返る」を開くと案内文がある |
| 4i | 書き出しのあと、もう一度「言葉にする」 | **読む** は前のタイトル／本文ではなく「いま写真を読んでいます…」から始まる。**オーナー PASS（2026-08-29）** |
...
| ステップ | 内容 | 結果 |
|----------|------|------|
| 0 | ターミナル起動 | PASS（2026-08-29） |
| 1 | git pull | PASS（2026-08-29） |
| 2 | Guided 起動 | PASS（2026-08-29） |
| 3 | 選ぶ・パラメータ表示 | PASS（2026-08-29） |
| 4 | 画面遷移・★必須・4i | PASS（2026-08-29） |
| 5 | 終了 Ctrl+C | PASS（2026-08-29） |
