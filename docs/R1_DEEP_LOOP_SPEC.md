...
スクリーニングアルゴリズムの数値閾値・実装手法は実装段階で詳細化する。本仕様は段の目的・順序・禁止事項・入出力・責任分界を固定する。
...
| 用途 | 一次ソース | 明示的に使わないもの |
|------|------------|----------------------|
| **スクリーニング**（Rating / 説明） | **JPEG 内**の Rating + Description（IPTC/XMP） | `.dop` サイドカー、`.xmp` サイドカー |
...
| スライス | 状態 | 内容 |
|----------|------|------|
| **R1′-A** | **第一波必須** | スクリーニング（M1/M2/M3）、JPEG への Rating／説明書き込み、監査ログ、確定画像へのカード／ログ |
...
| Rating | XMP/IPTC 等の評価値。スクリーニング段の結果の一次表現（人向け） |
...
- M2/M3 は軽量パス。フル講評（Phase2）はスクリーニングで走らせない。
...
2. スクリーニング実行・進捗・中断・ログ  
3. Lumina Review：Works（または確定フォルダ）を指定してカード／ログ  
...
**作らない:** スクリーニングのサムネ確認 UI、H1/H2/H3 専用画面、Works コピー機能。  
...
{works}/                         # DxO 等が書き出した先（パスはユーザー設定）
  IMG_0001.jpg                   # 任意
  IMG_0001_dev.jpg
  （本アプリはここを読んでLumina Review。コピーはしない）
```
...
| A7 | Works 上の `_dev.jpg`（または撮って出し）にLumina Review を付けられる | カード／ノートが辿れる |
...
| 今固定 | 実装／検証 |
|--------|------------|
| §0 メタ一次ソース（スクリーニング=JPEG Rating/Description、講評=画+撮影EXIF） | DxO 同期の合格／不合格（§0.1） |
...
| 日付 | 内容 |
|------|------|
| 2026-08-10 | 初版〜段階スクリーニング・ペアコピー・`_dev.jpg` |
| 2026-08-10 | **IPTC Rating／説明駆動に大改訂**。H1/H2 廃止、H3/W は DxO 等。Works コピーは本アプリ非対象。Rating0=除外マーク。確定は 3/4。同期は要検証 |
| 2026-08-11 | **§0 メタ一次ソースを確定方針として追記**（スクリーニング=JPEG Rating+Description、講評=画+撮影EXIF、同期成立時は dop/xmp 不使用）。検証チェックリスト追加 |
