# やり残し ToDo（Wave A/B/C 後）— 再レビュー確定版

更新日: 2026-08-13  
位置づけ: 初回ウォークスルー／UX 再レビュー由来のやり残しを、**オーナー再レビュー（2026-08-13）**で優先度・実装順を確定したもの。  
根拠: [`R1A_DESKTOP_WALKTHROUGH_BACKLOG.md`](R1A_DESKTOP_WALKTHROUGH_BACKLOG.md) / [`R1A_UX_IMPROVEMENT_PLAN.md`](R1A_UX_IMPROVEMENT_PLAN.md) / [`CURRENT_APP_MAP.md`](CURRENT_APP_MAP.md)

**完了済み（参考）:** Wave A/B/C。ホットフィックス H1–H3 / M1–M5 / L1–L5。**S4** LINE 統合実機 PASS（2026-08-12）。

**実装順の約束（専門家確認）:**

- P1 内は **上から順**（案内 → 役割一本化 → 最後に alias 削除）。`S5` を先にしない。
- **U2（P1）** はランチャー／README／起動案内の役割一本化まで。旧 `app_gui` の大規模統合は混ぜない。
- **P2-1**（モデル変更時）と **P2-2**（公開向け UI）は独立。モデル作業を UI 刷新の前提にしない。

---

## 実装順一覧（再レビュー確定）

### P1 — 次に実装する

| ID | 内容 | メモ |
|----|------|------|
| S1 | オリジナル → Works の置き方ガイドを常時出す（`_dev`／月フォルダ） | |
| S6 | スクリーニング失敗・中止時の「次の一手」一文 | |
| S7 | フォルダ名エラーを具体例つきで早く拒否 | |
| U2 | Photo AI 講評と Console の役割一本化（旧 C5） | **案内・ランチャーまで**（コード大規模統合はしない） |
| S5 | 旧名 alias 削除（`shortlist_*` 等） | **P1 の最後**。U2 で入口を固めてから |

### P2-LINE — LINE 単独（N1 Web UI に依存しない・先にやってよい）

| ID | 内容 | メモ |
|----|------|------|
| **N2（新規）** | LINE: カード送信後（または対話【1〜3】送信後）に画面下へ Quick Reply ボタンを出す | 設定の「いいね」切替と同じ Quick Reply パターン。デスクトップの同意 UI（旧 U7）とは別物 |

### P2-1 — AI モデル変更を検討するタイミングで

| ID | 内容 | メモ |
|----|------|------|
| Q2 | プロンプト審判語の回帰テスト強化 | モデル依存 |
| Q3 | Phase D 残課題の再評価 | モデル依存 |
| Q5 | H3 差分をプロンプト改善材料にするループ | モデル依存 |

### P2-2 — 公開向けの洗練 UI とまとめて

| ID | 内容 | メモ |
|----|------|------|
| **N1（新規）** | 開発 UI から公開向け洗練 UI へ（Web UI: FastAPI＋ブラウザ等を検討） | 親テーマ |
| U4 | 表層の評価語彙を対話語彙へ | 洗練 UI と一体 |
| Q4 | CRITIQUE_SUMMARY を「次の撮影への問い」寄りに | 同上 |
| U1 | カードで要約／問い上位・★二次（旧 C1） | 同上 |
| Q1 | カード描画で ★ を下げ要約を主役に | 同上 |
| Q7 | M2「相対熱量」をユーザー向け一言で | 同上 |
| S3 | ヘルプ／完了文の言い回し磨き（W5 後回し分） | 同上 |

### P3-1

| ID | 内容 | メモ |
|----|------|------|
| S8 | HIF 等 JPEG 以外の方針を決めて文書化 | |

### P3-2

| ID | 内容 | メモ |
|----|------|------|
| Q6 | レンズ将来拡張（`audience` 等）は仕様のみ | 実装は後 |

---

## 不要（再レビューで外したもの）

| ID | 内容 | 理由（オーナー） |
|----|------|------------------|
| U3 | Rating 0–4 と DxO 星の説明強化 | JPEG＝Lumina／RAW＝ユーザー評価で分離可能 |
| S2 | 撮って出しと `_dev` の同時 Review 選択 | 通常運用はどちらか一方 |
| U5 | 速い輪（R4′）の製品設計 | DxO での Rating 修正で達成 |
| U6 | 初回がスクリーニング偏重にならない導線 | ユーザーのワークフロー次第 |
| U7 | 同意 UI を作らず H3 使い方だけ決める | 専用 UI は不要。反応は LINE ならカード送信後の Quick Reply（Like 相当）で足りる想定 |

---

## テーマ別インデックス（参照用）

| テーマ | 残っている ID |
|--------|----------------|
| ストレス低減 | S1, S5, S6, S7, S8（S2/S4 は不要または完了） |
| Lumina Notes UX | U1, U2, U4, N1（U3/U5/U6/U7 は不要） |
| AI フィードバック質 | Q1, Q2, Q3, Q4, Q5, Q6, Q7 |

---

## 意図的にやらない（従来どおり）

| 項目 | 理由 |
|------|------|
| Works への自動コピー | 運用契約 |
| 長期 Phase1 content-hash キャッシュ | JPEG Description 正本 |
| GPT ストリーム前後半分割 | βでは複雑さ勝ち |
| 強制ウィザード（①〜⑤全体） | タブ独立と矛盾 |
| デスクトップの「そう思う／違う」専用 UI | 不要（上記 U7） |

---

## 変更履歴

| 日付 | 内容 |
|------|------|
| 2026-08-12 | 初版（3分類） |
| 2026-08-12 | S4 LINE PASS |
| 2026-08-13 | オーナー再レビュー反映: P1／P2-1／P2-2／P3・不要を確定。N1（洗練 UI）追加。実装順の約束を追記 |
