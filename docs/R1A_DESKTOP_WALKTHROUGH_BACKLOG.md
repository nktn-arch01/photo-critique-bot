...
2. スクリーニングのフォルダ名ゲート／直下 JPEG のみ  
3. 月一括時にイベント配下が黙って対象外  
4. Works フォルダ名が非 `YYYYMM` のときの `DesktopLogManager` 出力名（親に変な年次ログ）  
5. スクリーニング GUI が API キー未検証のまま M2/M3 開始可能  
6. `_dev` 優先で撮って出しを黙ってスキップ（両方にLumina Review を付けたい場合の UI なし）  
7. 「DxO修正後を記録」とLumina Reviewの順序・必須感の整理  
