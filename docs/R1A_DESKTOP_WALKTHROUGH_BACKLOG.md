...
| UX | 手順は通るが初心者には重い（二アプリ、Rating 意味、DxO hop）。Wave A で事前確認・見える化・語彙を改善（計画: `R1A_UX_IMPROVEMENT_PLAN.md`） |
...
4. **二アプリ・二ブランド名**  
   `Lumina Notes Console` と `Photo AI 講評` の役割分担を初心者向けに一本化／案内する。
...
計画の正本: [`R1A_UX_IMPROVEMENT_PLAN.md`](R1A_UX_IMPROVEMENT_PLAN.md)（2026-08-12 再レビュー後）。

| # | 内容 | 状態 |
|---|------|------|
| 1 | アプリ → DxO → アプリの往復と Works 規約（`_dev.jpg`、RAW 非配置）の学習コスト | **部分対応 Wave A4/A6**（タブ分離・文言）。B1 でステップ表示は任意 |
| 2 | スクリーニングのフォルダ名ゲート／直下 JPEG のみ | 運用契約のため維持。**B2 で任意のイベント順回し** |
| 3 | 月一括時にイベント配下が黙って対象外 | **対応済み C3UX**（ログ＋確認ダイアログ注意） |
| 4 | Works フォルダ名が非 `YYYYMM` のときの年次ログ崩れ | **対応済み M1** |
| 5 | スクリーニング GUI が API キー未検証のまま M2/M3 開始可能 | **対応 Wave A1** |
| 6 | `_dev` 優先で撮って出しを黙ってスキップ | **部分対応 Wave A2**（枚数・名前の可視化）。同時レビュー UI は後続 |
| 7 | 「DxO修正後を記録」と Lumina Review の順序・必須感 | **部分対応 Wave A3/A4**（終了時任意確認＋次の一手文言） |
...
| P1 | スクリーニングが `XXYYYYMM` / `XXYYYYMMDD_名前` を受け付ける（`XX`=機種2文字。実運用オリジナル。M1–M5 より上流） | **対応済み 2026-08-11** |
