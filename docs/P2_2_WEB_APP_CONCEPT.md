...
| `time_band` | 不定時法インスパイアの時間帯（下表。**選ぶ画面用**。GPS 無し時は timezone の代表座標） |
| `light_hint` | 太陽の方位・高度の**事実**（東の空／西の空、一日の前半／後半。講評はこれを覆さない。朝日・夕日等は入れない） |
...
**時間帯（`time_band`）— 不定時法準拠（画面）**

1. GPS + 撮影日時から **日出・日没**を天文計算し、タイムゾーンの**時計時刻**へ変換する（太陽時をそのまま日本時間と見なさない。**GPS 無し時は `timezone` の代表座標**）  
2. 夜との境界は **ブルーアワー開始**（太陽高度 約 −8°）。算出不能時のみ日出30分前／日没30分後  
3. その昼間を **7 等分**し、次のラベルを付与  
4. それ以外（夜間帯）は **`夜`** に一括

講評 API へは `time_band` の名前ではなく `light_hint`（方位＋高度）を渡す。画面の光がヒントと食い違うときは、強さ・色・影だけ画面優先。方位（東／西）は覆さない。プロンプト案の上限つき評価は [`eval/prompt_eval/README.md`](../eval/prompt_eval/README.md)。
