...
【時間帯ファクトの扱いに関する絶対ルール】
1. **[前提の固定]**: 撮影データとして示された【時間帯ファクト: {time_zone_fact}】を事実としての絶対的前提としてください。「朝日」を「夕日」、「午前」を「午後」と解釈・誤認した記述は一切禁止します。
2. **[時間帯依存ポエムの禁止]**: 「早朝の爽やかな空気」「夕暮れの哀愁」といった時間帯そのものを語るポエム的定型表現を連発することを固く禁止します。その時間帯の光（低い角度の光、伸びる影など）が被写体の質感や配置にどう作用しているかの具体描写に注力してください。
...
# 照合用情報
・ファイル名: {filename}
・撮影日時: {photo_datetime} 【時間帯ファクト: {time_zone_fact}】
・カメラ: {camera}
・レンズ: {lens}
・設定: {settings}
...
for idx, filename in enumerate(files, 1):
    ...
    # 時間帯判定ロジック（ファクトラベル）
    try:
        photo_hour = int(photo_datetime[11:13]) if len(photo_datetime) >= 13 else -1
        if 4 <= photo_hour < 8:
            time_zone_fact = "早朝・朝日（午前）"
        elif 8 <= photo_hour < 11:
            time_zone_fact = "午前（朝〜昼前）"
        elif 11 <= photo_hour < 14:
            time_zone_fact = "日中・正午前後"
        elif 14 <= photo_hour < 16:
            time_zone_fact = "午後（昼下がり）"
        elif 16 <= photo_hour < 19:
            time_zone_fact = "夕方・夕日（遅い午後）"
        elif 19 <= photo_hour or photo_hour < 4:
            time_zone_fact = "夜間・深夜"
        else:
            time_zone_fact = "時間帯不明"
    except Exception:
        time_zone_fact = "時間帯不明"
    ...
    print(f"   [撮影年] {photo_year}年 ({photo_datetime}) 【{time_zone_fact}】")
